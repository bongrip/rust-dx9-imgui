...
pub extern "stdcall" fn hk_end_scene(p_device: LPDIRECT3DDEVICE9) -> HRESULT {
  ...
  unsafe {
    ...
    let mut show = true;
    {
      ...
      Window::new(im_str!("emvhook [stable]"))
          .size([370.0, 100.0], imgui::Condition::FirstUseEver)
          .position([50.0, 50.0], imgui::Condition::FirstUseEver)
          .build(&ui, || {
            ui.button(im_str!("drop heist specific bag"), [350f32, 15f32]); // Hook to function
            ui.button(im_str!("secure heist specific bag(s)"), [350f32, 15f32]); // Sample: if ui.button(im_str!("Test"), [350f32, 15f32]) { println!("Hey!"); }
            ui.button(im_str!("spoof infamy"), [350f32, 15f32]);
          });
    }
...
pub unsafe fn hook_wnd_proc(hwnd: HWND) {
  O_WND_PROC = Some(
    mem::transmute(SetWindowLongPtrW(hwnd, GWLP_WNDPROC, hk_wnd_proc as i32))
  );
}
